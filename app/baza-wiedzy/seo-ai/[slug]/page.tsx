...
export async function generateMetadata({
  ...
  if (!entry) return {};
  return {
    title: `${entry.title} – SEO AI – Maks Biderman`,
    description: entry.description,
    alternates: {
      canonical: `/baza-wiedzy/seo-ai/${slug}`,
    },
  };
}

export default async function SeoAiEntryPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  try {
    const module = await import(`@/content/baza-wiedzy/seo-ai/${slug}.mdx`);
    Post = module.default;
  } catch {
    notFound();
  }
